...
def create_update_message(
    ...
    """
    assert new_risk_level <= message_uid_mask, f"{new_risk_level} passed bigger than 15"
...
def create_encounter_from_update_message(
    ...
    return EncounterMessage(
        ...
        # cannot properly deduct if the original encounter was an exposition
        _exposition_event=None,
    )
...
def find_encounter_match_score(
        ...
        # one tick per second, one roll per day
        ticks_per_uid_roll: TimeOffsetType = 24 * 60 * 60,
):
    ...
    uid_roll_count = \
        int((int(msg_new.encounter_time) -
             int(msg_old.encounter_time)) / ticks_per_uid_roll)
