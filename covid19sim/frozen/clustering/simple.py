...
class SimpleCluster(ClusterBase):
    ...
    messages: typing.List[EncounterMessage] = dataclasses.field(
    ...
    def create_cluster_from_message(
        ...
        return SimpleCluster(
            ...
            # once added, messages here should never be removed
            _unclustered_messages=[message],
        )
    ...
    def fit_encounter_message(self, message: EncounterMessage):
        ...
        # in this list, encounters may get updated
        self.messages.append(message)
        self._real_encounter_uids.append(message._sender_uid)
        self._real_encounter_times.append(message._real_encounter_time)
        # in this list, messages NEVER get updated
        self._unclustered_messages.append(message)
    ...
    def fit_update_message(self, update_message: UpdateMessage):
        ...
        # in this list, messages NEVER get updated
        self._unclustered_messages.append(update_message)
    ...
    def get_cluster_embedding(
        ...
        actual_risk_level = RiskLevelType(
            np.round(np.mean([m.risk_level for m in self.messages])))
        if old_compat_mode:
...
class SimplisticClusterManager(ClusterManagerBase):
    ...
    def __init__(
            self,
            # one tick per second, 14 days
            max_history_ticks_offset: TimeOffsetType = 24 * 60 * 60 * 14,
            ...
            # let's hope no user ever reaches 1000 simultaneous clusters
    ...
    def _add_encounter_message(self, message: EncounterMessage, cleanup: bool = True):
        ...
        if matched_cluster is not None:
            ...
            new_cluster = SimpleCluster.create_cluster_from_message(
                message, self.next_cluster_id)
            self.next_cluster_id = (
                self.next_cluster_id + 1) % self.max_cluster_id
        ...
        if matched_cluster is not None:
            ...
            if self.add_orphan_updates_as_clusters:
                ...
                raise AssertionError(
                    f"could not find any proper cluster match for: {message}")
