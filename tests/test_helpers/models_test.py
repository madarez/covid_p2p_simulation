    ...
    def test_run(self):
        ...
        with TemporaryDirectory() as preprocess_d:
            ...
            self.assertEqual(len(days_output), n_days -
                             ExpConfig.get('INTERVENTION_DAY'))
            output = [[]] * len(days_output)

            for day_output in days_output:
                pkls = glob.glob(f"{day_output}*/daily_human-*.pkl")
                pkls.sort(key=lambda p: (
                    int(p.split(os.path.sep)[-3]), int(p.split(os.path.sep)[-2])))
                day_humans = []
                for pkl in pkls:
                    with open(pkl, 'rb') as f:
                        ...
                        self.assertEqual(
                            day_human['current_day'], day_humans[0]['current_day'])
                self.assertGreaterEqual(len(day_humans), n_people)
                output[day_humans[0]['current_day'] -
                       ExpConfig.get('INTERVENTION_DAY')] = day_humans
            ...
            for current_day, day_output in zip(range(ExpConfig.get('INTERVENTION_DAY'), n_days),
                ...
                for h_i, human in enumerate(day_output):
                    ...
                    if current_day == ExpConfig.get('INTERVENTION_DAY'):
                        ...
                        prev_observed = output[output_day_index -
                                               1][h_i]['observed']
                        prev_unobserved = output[output_day_index -
                                                 1][h_i]['unobserved']
                    ...
                    self.assertEqual(
                        observed['reported_symptoms'].shape, (14, 28))
                    if observed['candidate_encounters'].size:
                        ...
                        self.assertGreaterEqual(
                            observed['candidate_encounters'][:, 0].min(), 0)  # cluster id
                        self.assertGreaterEqual(
                            observed['candidate_encounters'][:, 1].min(), 0)  # risk level
                        self.assertLess(
                            observed['candidate_encounters'][:, 1].max(), 16)  # risk level
                        self.assertGreaterEqual(
                            observed['candidate_encounters'][:, 2].min(), 0)  # encounters
                        self.assertGreaterEqual(
                            observed['candidate_encounters'][:, 3].min(), 0)  # day idx
                    ...
                    # Multihot encoding
                    self.assertIn(
                        observed['preexisting_conditions'].min(), (0, 1))
                    self.assertIn(
                        observed['preexisting_conditions'].max(), (0, 1))
                    ...
                    #  'mild', 'moderate', 'runny_nose', 'severe', 'trouble_breathing']
                    self.assertEqual(
                        unobserved['true_symptoms'].shape, (14, 28))
                    ...
                    # Multihot encoding
                    self.assertIn(
                        unobserved['true_preexisting_conditions'].min(), (0, 1))
                    self.assertIn(
                        unobserved['true_preexisting_conditions'].max(), (0, 1))
                    ...
                    if unobserved['is_exposed'] or unobserved['is_recovered']:
                        self.assertNotEqual(
                            unobserved['is_exposed'], unobserved['is_recovered'])
                    ...
                    self.assertGreaterEqual(
                        unobserved['true_age'], observed['age'])
                    # If observed['sex'] is set, unobserved['true_sex'] should also be set to the same value
                    self.assertGreaterEqual(
                        unobserved['true_sex'], observed['sex'])

                    if prev_observed:
                        self.assertTrue(
                            (observed['reported_symptoms'][1:] == prev_observed['reported_symptoms'][:13]).all())
                        if observed['candidate_encounters'].size and prev_observed['candidate_encounters'].size:
                            ...
                            # Get the last 13 days excluding today
                            current_day_mask = observed['candidate_encounters'][:,
                                                                                3] < current_day
                            # Get the last 13 days including relative today (of yesterday)
                            prev_day_mask = prev_observed['candidate_encounters'][:,
                                                                                  3] > current_day - 14
                            masked = observed['candidate_encounters'][current_day_mask][:, (
                                0, 1, 3)]
                            prev_masked = prev_observed['candidate_encounters'][prev_day_mask][:, (
                                0, 1, 3)]
                            offset = 0
                            for i in range(prev_masked.shape[0]):
                                for j in range(i+offset, masked.shape[0]):
                                    ...
                                    self.assertFalse(False,
                                                     ...
                                                     f"in current day.")
                        self.assertTrue(
                            (observed['test_results'][1:] == prev_observed['test_results'][:13]).all())
                        ...
                        self.assertTrue(
                            (unobserved['true_symptoms'][1:] == prev_unobserved['true_symptoms'][:13]).all())
                        check = unobserved['infectiousness'][1:
                                                             ] == prev_unobserved['infectiousness'][:13]
                        self.assertTrue(check if isinstance(
                            check, bool) else check.all())
                        ...
                        if unobserved['is_exposed'] != prev_unobserved['is_exposed']:
                            if unobserved['is_exposed']:
                                ...
                                self.assertEqual(
                                    unobserved['exposure_day'], None)

                        self.assertTrue((unobserved['true_preexisting_conditions'] ==
                                         prev_unobserved['true_preexisting_conditions']).all())
                        self.assertEqual(
                            unobserved['true_age'], prev_unobserved['true_age'])
                        self.assertEqual(
                            unobserved['true_sex'], prev_unobserved['true_sex'])
